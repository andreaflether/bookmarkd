...
RSpec.describe 'Folder - JS', type: :request do
  ...
  describe 'PUT /folders/pin_folder/:id, format: :js' do
    ...
    context 'when folder is NOT pinned' do
      before { put toggle_pin_folder_path(unpinned_folder), params: { session: login_as(user), format: :js } }
    ...
    context 'when folder is pinned' do
      before { put toggle_pin_folder_path(pinned_folder), params: { session: login_as(user), format: :js } }
    ...
    context 'when user already has 15 pinned folders and tries do pin another one' do
      let!(:folders) { create_list(:folder, 15, pinned: true, user: user) }
      let(:invalid_folder) { create(:folder, user: user) }

      before { put toggle_pin_folder_path(invalid_folder), params: { session: login_as(user), format: :js } }
